...
namespace
{
    ...
    void LoopBasedOnMinibatches(const std::wstring& name, const DeviceDescriptor& device, DistributedTrainerPtr distributedTrainer, size_t rank, const FeedForwardClassifier& classifier)
    {
        UNUSED(rank);
    ...
    void LoopBasedOnSamples(const std::wstring& name, const DeviceDescriptor& device, DistributedTrainerPtr distributedTrainer, size_t rank, const FeedForwardClassifier& classifier)
    {
        UNUSED(rank);
...
void TestFrameMode()
{
    ...
    std::map<std::wstring, DistributedTrainerPtr> trainers;
    trainers[L"simple"] = CreateDataParallelDistributedTrainer(MPICommunicator(), false);

    if (Is1bitSGDAvailable())
    {
        trainers[L"1bitsgd"] = CreateQuantizedDataParallelDistributedTrainer(QuantizedMPICommunicator(true, true, 32), false);
        trainers[L"blockmomentum"] = CreateBlockMomentumDistributedTrainer(MPICommunicator(), 1024);
    }
    ...
    std::vector<std::function<void(const std::wstring&, const DeviceDescriptor&, DistributedTrainerPtr, size_t, const FeedForwardClassifier&)>> loops;
    ...
    for (auto t : trainers)
    {
        for (auto device : devices)
        {
            for (auto loop : loops)
            {
                auto communicator = t.second->GetCommunicator();
                size_t rank = communicator->CurrentWorker().m_globalRank;

                communicator->Barrier();
                loop(t.first, device, t.second, rank, BuildFeedForwardClassifer(device));
                communicator->Barrier();
            }
        }
    }
